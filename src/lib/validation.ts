...
export function validate_bridge_url(
  bridge_url: string,
  is_staging?: boolean,
): ValidationResponse {
  try {
  ...
  if (is_staging && ['localhost', '127.0.0.1'].includes(test_url.hostname)) {
    console.log(
      'Using staging app_id with localhost bridge_url. Skipping validation.',
    );
    return { valid: true };
  }
  ...
  if (
    !test_url.hostname.endsWith('.worldcoin.org') &&
    !test_url.hostname.endsWith('.toolsforhumanity.com')
  ) {
    console.warn(
